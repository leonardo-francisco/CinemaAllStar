appear = (function () {
    ...
    return function (obj) {
        return (function (obj) {
            var initd = false, elements = [], elementsLength, reappear = [], appeared = 0, disappeared = 0, timer, deltaSet, opts = {}, done; function debounce(fn, delay) { return function () { var self = this, args = arguments; clearTimeout(timer); timer = setTimeout(function () { fn.apply(self, args); }, delay); }; }
            function checkAppear() {
                if (scroll.delta < opts.delta.speed) { if (!deltaSet) { deltaSet = true; doCheckAppear(); setTimeout(function () { deltaSet = false; }, opts.delta.timeout); } }
            ...
            function doCheckAppear() {
                ...
                elements.forEach(function (n, i) {
                    if (n && viewable(n, opts.bounds)) {
                        if (reappear[i]) {
                            reappear[i] = false; appeared++; if (opts.appear) { opts.appear(n); }
                ...
                }); if (!opts.reappear && (!opts.appear || opts.appear && appeared === elementsLength) && (!opts.disappear || opts.disappear && disappeared === elementsLength)) { done = true; removeListeners(); if (opts.done) { opts.done(); } }
            }
            function init() {
                if (initd) { return; }
                ...
                if (els) {
                    elementsLength = els.length; for (var i = 0; i < elementsLength; i += 1) { elements.push(els[i]); reappear.push(true); }
                    begin();
                }
            }
            return function (obj) {
                obj = obj || {}; opts = { init: obj.init, elements: obj.elements, appear: obj.appear, disappear: obj.disappear, done: obj.done, reappear: obj.reappear, bounds: obj.bounds || 0, debounce: obj.debounce || 50, delta: { speed: obj.deltaSpeed || 50, timeout: obj.deltaTimeout || 500 } }; addEventListener('DOMContentLoaded', init, false); var isIE10 = false; if (Function('/*@cc_on return document.documentMode===10@*/')()) { isIE10 = true; }
